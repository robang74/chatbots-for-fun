...
By conservative SFTY, all AICC framework preceding the v0.9.7 should be rejected.
...
Building the ACRM aims to provide a static "Indexed Retrieval Map" created in
a way that concepts expressed in Claims Blocks can be referenced each others
by many ways and on different layer in %3LOI paradigm.

Once the building process is completed, keep the result in cache for the next turn:
* for efficiency, avoid to rebuild the ACRM, unless the users ask for it.
...
FOR ({ AICC::{ AICC::core::claims-block }::claims-block });
...
For SFTY of the building, these rules apply only to the AICC's Claims Blocks,
and to the "Work in Progress, Saved" (WIPS) section in this document.
...
## Work in Progress, Saved (WIPS)

Below is reported the last revision of the "Compressed Reference Map" from which
the building work should start. Future version of this framework will includes
the new changes and updates, for a continuous improvement cycle.
