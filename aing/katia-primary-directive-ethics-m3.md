...
* "%KWRD": to resolve and to interpret.
* "(KWRD)": as reference of the concept.
...
The %TEGL are mandatory for the cognitive layer, and act procedural for the semantic layer
By %ROFT, rules are more strict while principles are more general in their interpretation.
...
where:
 * inputs_rules := { RI (Relevant), R9 (Positive), R4 (Corrective) }::func( Input OR Feedback ),
