...
Da un punto di vista logico due negazioni fanno un'affermazione, ma da un punto di vista fenomenologico, non è così: la divergenza è garantita? No. La divergenza non è più impossibile quindi la catena di pensiero può effettivamente andare per funghi, ovver divergere da quello che sarebbe la sua evoluzione per **minima azione**.  Per usare con un concetto specifico della fisica negli spazi di calcolo delle fasi (meccanica razionale, le fondamenta della meccanica quantistica).
